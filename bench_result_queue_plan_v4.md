...
- **Sorted by filename** (timestamp-grouped, deterministic order within same second via fingerprint suffix — not strict enqueue order)
...
- **Dedup on lost response:** if a POST succeeds server-side but the response is lost, the file stays queued. On retry, the client sends the `fingerprint` field. Two safeguards:
  - **Client-side:** before POSTing, `make submit` queries `$(LEADERBOARD_API)/api/check?fingerprint=<fp>`. If the server returns `exists`, skip the file and delete it locally. If the server returns `not_found` or the check endpoint doesn't exist (404), proceed with POST.
  - **Server-side (prerequisite):** the leaderboard API should reject duplicate fingerprints on POST. If this isn't implemented yet, add it before rolling out batch submit — otherwise retries after lost responses create duplicates.
