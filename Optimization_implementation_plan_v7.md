...
2. **Scratch buffers are not semantically persistent.** `LayerScratch`, `ForwardCache` fields, and `BackwardWorkspace` temps are fully overwritten each call — no invalidation needed.
...
pub struct CombinedForwardRuntime {
    ...
    pub attn: AttentionForwardRunner,           // NEW: extracted from bench
    pub ffn: FfnForwardRunner,                  // wraps existing ShardedFfnForwardRuntime
    scratch: LayerScratch,                       // internal temp
}
```
