...
submit:
	...
	for f in $$(ls target/bench-results/*.json | sort); do \
		...
		CHECK=$$(curl -s -o /dev/null -w "%{http_code}" \
			"$(LEADERBOARD_API)/api/check?fingerprint=$$FP" 2>/dev/null || echo "000"); \
		if [ "$$CHECK" = "200" ]; then \
...
- **Fingerprint preflight:** before POSTing, queries `/api/check?fingerprint=<fp>`. Behavior by response:
  - `200` → already exists, skip file and delete locally
  - `404` (endpoint not implemented yet) → proceed with POST
  - `5xx` / timeout / invalid response / `000` (curl failure) → proceed with POST (server-side dedup is the backstop; preflight is best-effort, not a gate)
