...
fn metal_adam_matches_cpu_small() {
    ...
    step_fused(&mut p_cpu, &grad, &mut m_cpu, &mut v_cpu, 1, lr, b1, b2, eps, wd, gs);

    // GPU
    let mut p_gpu = param_init.clone();
    let mut m_gpu = m_init.clone();
    let mut v_gpu = v_init.clone();
    metal.step(&mut p_gpu, &grad, &mut m_gpu, &mut v_gpu, 1, lr, b1, b2, eps, wd);

    let pe = rel_err(&p_cpu, &p_gpu, "param");
    let me = rel_err(&m_cpu, &m_gpu, "m");
    let ve = rel_err(&v_cpu, &v_gpu, "v");
    println!("PASS: Metal Adam matches CPU (n={n}, max rel_err: param={pe:.2e}, m={me:.2e}, v={ve:.2e})");
}
...
fn metal_adam_batch_multi_step() {
    ...
    let grad: Vec<f32> = (0..n).map(|i| ((i * 7 + 3) % 1000) as f32 * 0.001 - 0.5).collect();
    let param_init: Vec<f32> = (0..n).map(|i| ((i * 13 + 7) % 1000) as f32 * 0.002 - 1.0).collect();
    ...
    for t in 1..=5 {
        step_fused(&mut p_cpu, &grad, &mut m_cpu, &mut v_cpu, t, lr, b1, b2, eps, wd, gs);
    ...
    println!("PASS: Metal Adam batch matches CPU after 5 steps (n={n}, max rel_err: param={pe:.2e}, m={me:.2e}, v={ve:.2e})");
}
...
fn metal_adam_zero_weight_decay() {
    ...
    let mut m_cpu = vec![0.0f32; n];
    let mut v_cpu = vec![0.0f32; n];
    step_fused(&mut p_cpu, &grad, &mut m_cpu, &mut v_cpu, 1, lr, b1, b2, eps, 0.0, gs);

    let mut p_gpu = param_init.clone();
    let mut m_gpu = vec![0.0f32; n];
    let mut v_gpu = vec![0.0f32; n];
    metal.step(&mut p_gpu, &grad, &mut m_gpu, &mut v_gpu, 1, lr, b1, b2, eps, 0.0);
