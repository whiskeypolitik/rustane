...
fn profile_config(cfg: &ModelConfig, name: &str) {
    ...
    println!("Model: {}L {}D {}H seq={} vocab={} — ~{:.0}M params\n",
             cfg.nlayers, cfg.dim, cfg.hidden, cfg.seq, cfg.vocab,
    ...
    for run in 0..3 {
        ...
        if run == 1 {
            ...
            println!("\n  Category breakdown:");
            println!("    ANE compute:    {:>6.2}ms ({:.0}%)", ane_ms, ane_ms / total * 100.0);
            println!("    IOSurf staging: {:>6.2}ms ({:.0}%)", stage_ms, stage_ms / total * 100.0);
            println!("    IOSurf read:    {:>6.2}ms ({:.0}%)", read_ms, read_ms / total * 100.0);
            println!("    CPU (rmsnorm):  {:>6.2}ms ({:.0}%)", cpu_ms, cpu_ms / total * 100.0);
        }
    }
