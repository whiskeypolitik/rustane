...
fn deterministic_signal(len: usize, scale: f32) -> Vec<f32> {
    (0..len)
        .map(|i| (((i * 31 + 7) % 97) as f32 - 48.0) * scale)
        .collect()
}
...
fn setup_case() -> (
    ...
    let dy = deterministic_signal(cfg.dim * cfg.seq, 0.005);
...
#[test]
fn sharded_ffn_backward_correctness_suite() {
    let (cfg, kernels, weights, cache, dy) = setup_case();
    let mut runtime = ShardedFfnBackwardRuntime::compile(&cfg, 4).expect("sharded FFN runtime");
    ...
    layer::backward_into_sharded_serial(
        &cfg,
        &kernels,
        &weights,
        &cache,
