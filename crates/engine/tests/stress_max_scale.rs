...
fn try_scale(name: &str, cfg: &ModelConfig) -> bool {
    ...
    println!("\n--- {name}: {:.2}B params, {}d/{}h/{}L ---", params, cfg.dim, cfg.hidden, cfg.nlayers);
    println!("    ffnFused IOSurf: {:.0} MB, weight mem: {:.1} GB, total est: {:.1} GB",
    ...
    let kernels = match std::panic::catch_unwind(|| CompiledKernels::compile(cfg)) {
        Ok(k) => k,
        Err(e) => {
            println!("PANIC: {:?}", e.downcast_ref::<String>().map(|s| s.as_str()).unwrap_or("unknown"));
            return false;
        }
    };
...
fn find_max_scale() {
    ...
    for (name, cfg) in &configs {
        if !try_scale(name, cfg) {
            ...
            println!("Breaking config: {}d/{}h/{}L", cfg.dim, cfg.hidden, cfg.nlayers);
            return;
        }
