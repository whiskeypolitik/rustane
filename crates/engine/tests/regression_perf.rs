...
fn run_deterministic(cfg: &ModelConfig, steps: u32) -> (Vec<f32>, f32, f32, f32) {
    ...
    {
        grads.zero_out();
        let _ = full_model::forward_ws(cfg, &kernels, &weights, &tokens, &targets, tc.softcap, &mut fwd_ws);
...
fn correctness_gpt_karpathy() {
    ...
    println!("  loss: {:.4} → {:.4} (delta={:+.4})", losses[0], losses.last().unwrap(), losses.last().unwrap() - losses[0]);
