...
fn main() {
    ...
    let training_parallel =
        TrainingParallelOptions::from_env_for_cfg(&cfg).unwrap_or_else(|message| {
            eprintln!("{message}");
            std::process::exit(1);
        });
